...
- (void)isOpen
{
    ...
    if(authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied){
        ...
        YBAlertView *alerView = [[YBAlertView alloc] initWithTitle:@"提示" contentText:@"您的相册权限未开启,请到设置-Hearer-相册,开启相册权限？" leftButtonTitle:@"取消" rightButtonTitle:@"前往" leftBlock:^{
        } righBlock:^{
...
- (void)setupLayout
{
    ...
    [self.btnClose setImage:[UIImage Uzys_imageNamed:@"关闭键-拷贝"/*appearanceConfig.closeImageName*/] forState:UIControlStateNormal];
    ...
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kFullWidth, 0.5)];
...
- (void)setupOneMediaTypeSelection
{
    ...
    else
    {
        ...
        else if(_maximumNumberOfSelectionVideo == 0)
        {
            ...
            self.labelSelectedMedia.hidden = NO;
            if(_maximumNumberOfSelection >1)
                self.labelSelectedMedia.text = [NSString stringWithFormat:@"最多选择%ld张图片",_maximumNumberOfSelection];
            else
...
- (void)setupCollectionView
{
    ...
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 64, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 64-44) collectionViewLayout:layout];
    ...
    switch (btn.tag) {
        case kTagButtonCamera:
        {
            ...
            if(authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied){
                ...
                YBAlertView *alerView = [[YBAlertView alloc] initWithTitle:@"提示" contentText:@"您的相机权限未开启,请到设置-Hearer-相机,开启相机权限？" leftButtonTitle:@"取消" rightButtonTitle:@"前往" leftBlock:^{
                } righBlock:^{
                    NSURL *url = [NSURL URLWithString:UIApplicationOpenSettingsURLString];
