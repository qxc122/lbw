...
- (void)setPaoMaDeng{
    if (!self.horizontalMarquee && [ChatTool shareChatTool].basicConfig.room_msg.length) {
        ...
        horizontalMarquee.font = [UIFont systemFontOfSize:17];
        [self.view addSubview:horizontalMarquee];
        
        CGFloat widthNULL = 3.8;
        NSString *msg = [ChatTool shareChatTool].basicConfig.room_msg;
        CGSize tmp = [msg sizeWithFont:[UIFont systemFontOfSize:17] andMaxSize:CGSizeMake(MAXFLOAT, MAXFLOAT)];
        ...
        self.messageTableView.contentInset = UIEdgeInsetsMake(20, 0, 0, 0);
    }
}
