...
- (void)drawRect:(CGRect)rect {
  ...
  self.voiceButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleTopMargin;
    if (![self.zhibojian isEqualToString:@"1"]) {
        ...
        [self.voiceButton setImage:[UIImage imageNamed:LIVECHATPNG] forState:UIControlStateNormal];
        self.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.2];
        self.textView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.3];
  ...
  self.recordAnimationView=[[JCHATRecordAnimationView alloc]initWithFrame:CGRectMake((kApplicationWidth-140)/2, (SCREENHEIGHT -kNavigationBarHeight - kTabBarHeight - 140)/2, 140, 140)];
